...
public class AppController {
	...
	public String redirectToDetailedPost(ModelMap model) {
		if (model.containsAttribute("user_id")) {
			...
			model.addAttribute("replies", replyService
					.findSortedReplies(postService.getCurrentPostID()));
			model.addAttribute("reply", new Reply());
			model.addAttribute("recursiveReply", new Reply());
			model.addAttribute("editReply", new Reply());

			if (replyService.getStatus() == 1) { // Reply
				int newDepth = replyService.getCurrentlyWorkingReply()
						.getDepth() + 1;
	...
	public String saveReply(@Valid Reply reply, BindingResult result,
			ModelMap model) {
		if (model.containsAttribute("user_id")) {
			...
			else if (replyService.getStatus() == 1
					|| replyService.getStatus() == 0)
			...
			model.addAttribute("replies",
					replyService.findSortedReplies(reply.getPost_id()));
			model.addAttribute("post", postService.getCurrentPost());
	...
	private FieldError createError(String errorProperty,
			String messageResourceBundle, String data) {
		FieldError error = new FieldError("user", errorProperty,
				messageSource.getMessage(messageResourceBundle,
						new String[] { data }, Locale.getDefault()));
		return error;
	}
}
